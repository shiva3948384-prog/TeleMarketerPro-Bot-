...
import { getDB, syncDB } from './lib/firebaseAdmin.js';
  ...
  try {
    ...
    if (cloudData) {
      ...
      if (content && content.trim()) {
        ...
        db = { ...db, ...data };
        await syncDB(db); // Migration to Firebase
        console.log('✅ DB migrated from local to Firebase.');
      }
    }
    return db;
  } catch (e) {
...
const saveDB = async (data?: any) => {
  ...
  try {
    ...
    console.log(`✅ DB synced to Firebase. Accounts: ${db.accounts.length}`);
  } catch (e) {
...
const startApp = async () => {
  // 1. Immediately bind to PORT 3000 to satisfy platform health checks
  // We do NOT await anything before this to ensure the preview gets a response
  ...
  (async () => {
    try {
      ...
      console.log(`✅ Database ready. Accounts: ${db.accounts.length}`);
      ...
      // 4. Launch bots (non-blocking)
      const launchMainBot = async () => {
        try {
          if (!BOT_TOKEN || BOT_TOKEN.trim() === '') {
            mainBotStatus = 'error';
            mainBotError = 'BOT_TOKEN is missing or empty in Settings -> Secrets';
            console.error(`❌ Main Bot: ${mainBotError}`);
            return false;
          }
          
          console.log(`Initializing Main Bot (Token Length: ${BOT_TOKEN.length})...`);
          bot = new Telegraf(BOT_TOKEN);
          
          console.log('Attempting to fetch main bot info...');
          ...
          bot.launch().catch(err => {
            mainBotStatus = 'error';
            mainBotError = `Launch Error: ${err.message || String(err)}`;
            console.error('❌ Main Bot launch error:', mainBotError);
          });
          ...
          if (errMsg.includes('401')) mainBotError = '401 Unauthorized: Invalid Token';
          else mainBotError = errMsg;
          return false;
        }
      };
      ...
      const launchLoggerBot = async () => {
        try {
          if (!LOGGER_BOT_TOKEN || LOGGER_BOT_TOKEN.trim() === '') {
            loggerBotStatus = 'error';
            loggerBotError = 'LOGGER_BOT_TOKEN is missing or empty';
            return false;
          }
      ...
      const mainBotOk = await launchMainBot();
      const loggerBotOk = await launchLoggerBot();

      if (mainBotOk || loggerBotOk) {
        console.log('✅ Bot initialization sequence completed');
        if (mainBotOk && loggerBotOk) {
          logToAdmin('🚀 *TeleMarketerPro Bot & Logger Bot are now ONLINE!*');
        }
      } else {
        console.error('❌ Both bots failed to launch. Check your tokens.');
      }
      
      // Initialize clients
      initClients().catch(err => console.error('❌ Error during initClients:', err));

      // Periodic Profile Check for ALL accounts
      setInterval(async () => {
        console.log('Running periodic profile branding check...');
          ...
          try {
            ...
            if (!client || !client.connected) {
              client = new TelegramClient(new StringSession(acc.session), API_ID, API_HASH, {
                ...
                timeout: 30000,
                useWSS: true,
                deviceModel: 'TeleMarketerPro Bot Desktop',
                systemVersion: 'Linux',
                appVersion: '1.5.0',
              });
            ...
            const updated = await enforceProfile(client, acc.userId, acc.phone);
            if (updated) {
              console.log(`[Branding] Profile updated/enforced for ${acc.phone}`);
            }
            ...
            console.error(`Error in periodic branding check for ${acc.phone}:`, e);
          }
          await new Promise(resolve => setTimeout(resolve, 3000)); // 3s delay between accounts
        }
      }, 10 * 60 * 1000); // Every 10 minutes

    } catch (err) {
      console.error('❌ Error in background startup:', err);
    }
  })();
};
...
const withRetry = async <T>(fn: () => Promise<T>, label: string, maxRetries: number = 2): Promise<T> => {
  let lastError: any;
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      if (err.message === 'TIMEOUT' || err.message?.includes('Timeout')) {
        console.log(`[Retry] ${label} failed (attempt ${i + 1}/${maxRetries}). Retrying in 2s...`);
...
const sendVerificationCode = async (ctx: any, userId: number, phone: string) => {
  ...
  try {
    const client = new TelegramClient(new StringSession(''), API_ID, API_HASH, {
      ...
      timeout: 60000,
      useWSS: true,
      deviceModel: 'TeleMarketerPro Bot Login',
      systemVersion: 'Linux',
      appVersion: '1.5.0',
    });
...
const startAutoReply = async (account: any) => {
  ...
  try {
    const client = new TelegramClient(new StringSession(account.session), API_ID, API_HASH, {
      ...
      timeout: 60000, 
      useWSS: true,
      deviceModel: 'TeleMarketerPro Bot AutoReply',
    });
